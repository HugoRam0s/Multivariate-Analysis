##############################################################################################################################
# SUPERVISED LEARNING USING CLUSTER LABELS
##############################################################################################################################
...
#Load the data 
diabetes_clusters <- read.csv("C:\\Users\\mcsam\\Desktop\\5ano\\Analise Multivariada\\projeto\\data\\processed\\Unsupervised_labeled.csv")
head(diabetes_clusters)

# Check the cluster labels distribution
class_counts <- table(diabetes_clusters$cluster)
print(class_counts)

# Change labels back to original (from 1,2,3 to 0,1,2)
diabetes_clusters$cluster <- diabetes_clusters$cluster - 1
class_counts <- table(diabetes_clusters$cluster)
print(class_counts)

# Convert cluster variable to factor
diabetes_clusters$cluster <- as.factor(diabetes_clusters$cluster)

# Set seed for reproducibility
set.seed(60)

# Partition data for training and testing
partition <- createDataPartition(diabetes_clusters$cluster, p = 0.60, list = FALSE)


# Create data frames for training and testing
train_data <- diabetes_clusters[partition, ]
test_data <- diabetes_clusters[-partition, ]
...
accuracy <- confusionMatrix(predictions1, test_data$cluster)
lr_accuracy <- accuracy$overall['Accuracy']
print(paste("Accuracy:", lr_accuracy))


# Calculate metrics from the confusion matrix
cm1 <- confusionMatrix(predictions1, test_data$cluster)
